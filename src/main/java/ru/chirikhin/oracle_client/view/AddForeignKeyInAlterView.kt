...
class AddForeignKeyInAlterView(columnSettings: ObservableList<Column>,
                               constraints : ObservableList<Constraint>,
                               databaseRepresentation: DatabaseRepresentation,
                               tablespace: String,
                               nameOfTable: String) : View() {
        ...
        form {
            ...
            button("Add new foreign key") {
                action {
                    ...
                    try {
                        val constraint = Constraint.ForeignKey(constraintNameTextField.text, srcColumnComboBox.value.name, destTableComboBox.value.name,
                                destColumnComboBox.value)

                        fire(EventAddConstraint(nameOfTable, constraint))
